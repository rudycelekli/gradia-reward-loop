...
reused for the reward loop. Every run writes append-only frames linked by a SHA-256 chain and
a self-verifying manifest, so every reported number recomputes offline and any tamper is caught.
Schema is kept compatible with the Wind Tunnel evidence manifest (frames_schema v1).
"""
